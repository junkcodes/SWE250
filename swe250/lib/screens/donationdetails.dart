...
class _donationdetailsScreenState extends State<donationdetails> {
  donationController dc = donationController();
  List<dd> DD = [];
  _getData() async {
    await dc.listDonation(widget.ccn);
    for (final row in dc.list.rows) {
      await dc.getUser(int.parse(row.colAt(5)));
      for (final jow in dc.user.rows) {
        DD.add(new dd(row.colAt(1), jow.colAt(1),jow.colAt(2),row.colAt(2),row.colAt(4),row.colAt(3)));
      }
    }
    setState(() {});
  }
